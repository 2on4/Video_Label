    ...
    try:
        ...
        for file, meta in zip(files, metadatas):
            ...
            if extra_info and extra_info['is_extra']:
                # Determine show and season if possible
                ...
                # Numbering for multiple extras of same type
                ...
                if season:
                    ...
                    base_name = f"{clean_show} - S{season:02d} - {extra_type}"
                else:
                    extras_dir = Path(target) / "TV Shows" / clean_show / "Extras"
                    base_name = f"{clean_show} - {extra_type}"
                ...
                if not dry_run:
                    ...
                    if new_file.exists():
                        ...
                        if src_hash == dst_hash:
                            logging.info(f"Duplicate extra (identical): {file} == {new_file}, removing source.")
                            file.unlink()
                        # Compare quality and playability
                        ...
                        if (src_playable and not dst_playable) or (src_quality > dst_quality):
                            logging.info(f"Replacing lower quality or unplayable extra: {new_file}")
                            new_file.unlink()
                        else:
                            logging.info(f"Skipping duplicate extra (better or equal exists): {file}")
                            file.unlink()
                    else:
